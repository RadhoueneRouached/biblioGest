...
<body>
	...
	<div id="conteneur">
		...
		<div id="center">
			...
			<!-- CONTENU  -->
			<div id="content">
				<h2><fmt:message key='statistic' /></h2>
						
						<h4> <fmt:message key='numberOfBorrows' /> : ${nbET}</h4>
						<h4> <fmt:message key='numberOfDocuments' /> : ${l}</h4>
						<h4> <fmt:message key='numberOfMembers' /> : ${nta}</h4>
...
<div id="container">
  ...
  <table id="chartData">
    ...
    <tr>
      <th><fmt:message key='document' /></th><th><fmt:message key='NumberOfCopiesByDocument' /></th>
     </tr>
	<c:forEach items="${e}" var="element"> 

    <tr style="color: #0266c${element.getDocument().exemplaire}">
      <td>${element.getDocument().titre}</td><td>${element.getDocument().exemplaire}</td>
    </tr>
</c:forEach>  </table>
</div>
...
<center>
<fmt:message key='borrowPerMember' />
<dl style="width: 300px">
<c:forEach items="${ad}" var="element"> 

<dt>${element.getAdherent().nom} ${element.getAdherent().prenom} - ${element.getDocument().titre}</dt>
<dd><div id="data-one" class="bar" style="width: ${element.getDocument().exemplaire}">${element.getDocument().exemplaire}</div></dd>
</c:forEach>
</dl></center>
</body>
</html>